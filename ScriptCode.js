...
if (isMobile()) {
	console.log("I am on a shit box");
} else {
	console.log("I am on MOBLIE REEEAAAL");
}
...
function setupUI() {
	...
	modmenutitle.textContent = 'die hi💀💀😈';
	...
	const textnode = document.createTextNode("BBC PLAY,S SKIDED VERSON snay.io script.   ");
...
loading();
 function handleCanvasClickUp(event) {
	...
	if (event.button === 2) {
		// clicks on x4 split button
	}
	if (joystick is moved) 
	//any direction
	if (joystick is not being moved)
	//dont click on x4 split button
	//dont click on macro button
